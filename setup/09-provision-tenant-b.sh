#!/usr/bin/env bash
# 09-provision-tenant-b.sh -- Create Tenant B hosted cluster, MetalLB, and DNS.
set -euo pipefail
...
TENANT_KUBECONFIG="${SETUP_DIR}/.generated-tenant-b-kubeconfig"
...
METALLB_VIP="10.100.31.100"
RELEASE_IMAGE="quay.io/openshift-release-dev/ocp-release:4.20.18-multi"

echo "=== Provisioning Tenant B hosted cluster ==="

echo "--- Setting up infra namespace, NAD, and RBAC ---"
export KUBECONFIG="${VIRT_KUBECONFIG}"
oc apply -f "${DEMO_DIR}/manifests/tenant-b/namespace.yaml"
oc apply -f "${DEMO_DIR}/manifests/tenant-b/nad-"*.yaml
...
INFRA_KUBECONFIG_FILE="${SETUP_DIR}/.generated-infra-sa-kubeconfig-b"
...
oc apply -f "${DEMO_DIR}/manifests/tenant-b/metallb-ingress.yaml" --server-side=true --force-conflicts 2>&1 || true
...
oc apply -f "${DEMO_DIR}/manifests/tenant-b/metallb-ingress.yaml"
...
echo "--- Creating Route53 DNS records for Tenant B ---"
export KUBECONFIG="${HUB_KUBECONFIG}"
